...
## 一、微处理器的发展特点
1. 融入了大型机的体系结构特点
   - 新一代微处理器的设计融入了大型机的体系结构特点，如内部的存储管理、指令高速缓存和数据高速缓存、高度的并行性和流水线、大的寄存器组、多机处理接口甚至片上带有协处理器。
2. 采用数据和指令分开存储和读写的方法
   - 在新一代微处理器中，为了进一步提高运行速度，采用数据和指令分开存储和读写的方法，使用多个数据、地址总线，并使片内的存储管理部件和转换后备缓冲器与微处理器并行工作。
3. 采用精简指令系统计算机（RISC）体系结构。
   - 在新一代微处理器中，广泛采用精简指令系统计算机（RISC）体系结构。RISC技术可以缩短计算机的设计周期、提高设计可靠性，特别是有较高的性能价格比，不仅在大型机、巨型机得到应用，在微型机中也获得日益增多的应用。
4. 能全面支持多用户、多任务的操作系统，直接支持常用的高级语言。
5. 还具有很强的联网功能，符合有关标准规定，支持多种形式的网络。

## 二、80x86微处理器

### 80286

![80286微处理器结构图](./微机接口与通讯02-80x80及Pentium微处理器/80286微处理器结构图.png)

#### 80286结构

整个80286采用流水线作业方式，使各部件能同时并行地工作。
...
- 两种工作方式
  - 实地址方式
  - 保护虚地址方式
- 具有高效的任务转换功能，可以适应多用户、多任务的需要

### 80386

![80386微处理器结构图](./微机接口与通讯02-80x80及Pentium微处理器/80386微处理器结构图.png)
...
![80486微处理器结构图](./微机接口与通讯02-80x80及Pentium微处理器/80486微处理器结构图.png)
...
80486在Intel微处理器历史上首次采用了RISC技术。它有效地优化了微处理器的性能。采用RISC技术 并不意味着80486与80386等微处理器不兼容，实际上80486的指令并没有精简，强调的只是RISC技术。采用 RISC技术的目的，是要使80486达到一个时钟周期执行一条指令。事实上，80486能达到平均一个时钟周期执行1.2条指令。

##### 2. 采用了突发总线同外部RAM进行高速数据交换。

通常微处理器与RAM进行数据交换时，取一个地址 ，交换一个数据。采用突发总线后，每取得一个地址，便将这个地址及以后地址中的数据都一起参与,从而 大大加快了微处理器与RAM间的数据传送率。这种技术尤其适用于图形显示和网络应用，因为在这两种情况 下，所涉及的地址空间一般都是连续的。

##### 3. 微处理器中配置了8KB字节的高速缓存器(Cache)。

高速缓存采用4路相连的实现方案，具有较高 的命中率。高速缓存由指令和数据共用，若在高速缓中找不到所需数据，可访问外部的存储器，一次可从外 部调入16字节指令或数据。

##### 4 .微处理器内部还设置了一个数值协处理器。

这就使得80486不再需要片外80387的支持而直接具 有浮点数据处理能力。这个协处理器可以极高的速度进行浮点数运算，且与80387兼容。
80486在其高速缓存部件与协处理器之间设置有二条高速数据总线，这二条32位的总线也可作为一条64位的总线使用。高档80486芯片的数据总线宽度可达128位。如此宽的数据交换通道为数据高速处理提供了保证

## 三、Pentium微处理器

Pentium微处理器采用亚微米级的CMOS，实现了0.8um技术，一方面使器件的尺寸进一步减小，另一方面使芯片上集成的晶体管数达到310万个。在Pentium微处理器的体系结构上，采用了许多过去在大型机中才采用的技术，迎合了高性能微型机系统需要，其主要体现在超标量流水线设计、双高速缓存、分支预测、改善浮点运算等方面。

### Pentium Pro微处理器

Intel公司于1995年底推出的Pentium Pro微处理器，是第六代微处理器系列的第一个产品。Pentium Pro比 普通Pentium增加8条指令，与X86微处理器系列完全向容。Pentium Pro微处理器具有64位数据线、36位地址线。197平方毫米的芯片上集成了550万个晶体管。


### PentiumⅡ微处理器

1997年5月Intel公司正式推出Pentium Ⅱ微处理器。它是Pentium Pro的先进性与MMX多媒体增强技术相结合的新型第六代微处理器，采用0.35um的CMOS半导体技术，片内集成750万个半导体元件，片内有L1 Cache为 32KB，L2 Cache为512KB。

### Pentium Ⅲ微处理器

Intel公司于1999年1月正式宣布了Pentium Ⅲ处理器，并于2底正式上市。Pentium Ⅲ处理器采用0.25um的 CMOS半导体技术，处理器核心集成有950万个晶体管

## 四、双核微处理器

双核微处理器是指基于单个半导体的一个处理器芯片上拥有两个一样功能的处理器核心，也就是将两个物理处理器核心整合在一个内核中。
